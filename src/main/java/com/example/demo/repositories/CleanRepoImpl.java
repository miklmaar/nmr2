...
public class CleanRepoImpl implements IMaintenanceRepository {
    private Connection conn;

    public CleanRepoImpl(){
    ...
    public Motorhome readFirstFromQueue() {
        ...
        try{
            PreparedStatement ps = conn.prepareStatement(
                    "SELECT * FROM CleaningQueue ORDER BY Priority LIMIT 1;"
            );
    ...
    public Motorhome popFromQueue() {
        ...
        try{
            ...
            PreparedStatement ps1 = conn.prepareStatement(
                    "DELETE FROM CleaningQueue WHERE MotorhomeID=" + motorhome.getMotorhomeID()
            );
            ps1.executeUpdate();
        } catch(SQLException s){
            s.printStackTrace();
    ...
    public LinkedList<Motorhome> readList() {
        ...
        try{
            PreparedStatement ps = conn.prepareStatement(
                    "SELECT * FROM CleaningQueue ORDER BY Priority;"
            );
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
